...
// 1. Definizione della funzione del Tour
  ...
  img.onload = () => {
    ...
    // Impostazioni tipografiche premium e leggibili
    ctx.fillStyle = "#1c1917"; 
    ctx.textAlign = "center";
    ctx.font = "italic bold 110px sans-serif"; // Carattere ingrandito e nitido

    // 2. Scritta in Alto
    ctx.fillText("SoulBook", size / 2, size * 0.16);

    // 3. QR Code posizionato centralmente
    const qrSize = size * 0.55; // Dimensione ottimizzata per far respirare i testi
    const qrX = (size - qrSize) / 2;
    const qrY = (size - qrSize) / 2; // Centratura verticale perfetta
    ctx.drawImage(img, qrX, qrY, qrSize, qrSize);

    // 4. Scritta in Basso
    ctx.fillText("Foto & Dediche", size / 2, size * 0.88);

    URL.revokeObjectURL(url);
    
    // 5. Generazione del file PNG e Download
    const pngUrl = canvas.toDataURL("image/png");
    const downloadLink = document.createElement("a");
    downloadLink.href = pngUrl;
    downloadLink.download = `soulbook-premium-${profile?.slug || 'qr'}.png`;
    downloadLink.click();
  };
  img.src = url;
};
  ...
  if (navigator.share) {
    try {
      await navigator.share({
        title: `SoulBook - ${fullName || 'Memoria'}`,
        text: `Accedi alla storia di ${fullName || 'un nostro caro'} e lascia un pensiero su SoulBook.`,
        url: qrValue,
      });
    } catch (err) {
    ...
    try {
      ...
      alert("Link della pagina copiato negli appunti!");
    } catch (err) {
...
<div id="step-qr-card" className="bg-white p-8 rounded-2xl shadow-sm border border-stone-100 flex flex-col items-center justify-center">
  ...
  <div className="flex flex-col items-center gap-2 w-full">
    <p className="text-[9px] font-bold uppercase text-stone-300 tracking-widest mb-1">Gestione Profilo</p>
    
    {/* Contenitore Pulsanti Affiancati */}
    <div className="flex gap-2 w-full justify-center">
      {/* Tasto Download */}
      <button onClick={downloadQRCode} className="flex-1 px-3 py-2.5 bg-stone-50 hover:bg-stone-100 text-stone-500 text-[9px] font-black uppercase tracking-tight rounded-lg border border-stone-100 transition-all flex items-center justify-center gap-2">
      ...
      {/* Tasto Condividi */}
      <button onClick={shareLink} className="flex-1 px-3 py-2.5 bg-stone-900 hover:bg-stone-800 text-white text-[9px] font-black uppercase tracking-tight rounded-lg transition-all flex items-center justify-center gap-2 shadow-sm">
        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8.684 10.742l4.184-2.092m0 6.698l-4.184-2.092A3.5 3.5 0 1112 15.5a3.5 3.5 0 01-1.316-2.742m0-5.516A3.5 3.5 0 1112 4.5a3.5 3.5 0 01-1.316 2.742" />
        </svg>
        Condividi
      </button>
    </div>
