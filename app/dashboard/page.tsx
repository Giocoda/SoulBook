...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    ...
    // CARICAMENTO ORDINATO PER POSITION (Cruciale per il refresh)
  ...
  const saveProfileData = async () => {
    ...
    if (!error) alert("Modifiche salvate con successo!");
    setIsSaving(false);
  };
  ...
  const handleFileUpload = async (e: any, bucket: string) => {
    ...
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      const filePath = `${profile.owner_id}/${Date.now()}_${file.name}`;
      ...
      if (!uploadError) {
        ...
        if (bucket === 'avatars') {
          ...
          // Inserimento con posizione corretta in coda
  ...
  const deleteItem = async (item: any) => {
    ...
    try {
      if (item.type !== 'youtube' && item.type !== 'vimeo' && item.type !== 'tiktok') {
  ...
  // VIDEO PLAYER UNIVERSALE (In linea con la pagina pubblica)
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div className="md:col-span-2 bg-white p-8 rounded-2xl shadow-sm border border-stone-100 flex flex-col md:flex-row items-center gap-10">
                <div className="relative group">
                  ...
                  <label className="absolute -bottom-2 -right-2 bg-stone-800 text-white p-2.5 rounded-lg cursor-pointer hover:bg-stone-600 transition-all shadow-lg">
                    <input type="file" hidden onChange={(e) => handleFileUpload(e, 'avatars')} />
  ...
  <div className="flex flex-col gap-1">
    ...
    <div className="flex items-center gap-2">
      <input 
        ...
        className="flex-grow bg-stone-50 p-2 rounded-lg text-xs font-bold outline-none border border-stone-100 h-[38px]" 
      />
      {/* Questo div vuoto serve a bilanciare lo spazio del bottone "Sempre" */}
      <div className="hidden md:block w-[85px] shrink-0"></div>
    </div>
  </div>
  ...
  <div className="flex flex-col gap-1">
    ...
    <div className="flex items-center gap-2">
      ...
      <button 
        ...
        style={{ 
          backgroundColor: `${profile.accent_color}15`, 
          color: profile.accent_color,
          borderColor: `${profile.accent_color}40` 
        }}
        className="shrink-0 w-[85px] h-[38px] rounded-lg text-[8px] font-black uppercase tracking-widest border transition-all hover:opacity-70 flex items-center justify-center gap-1"
      >
        <span className="text-xs">∞</span> Per Sempre
      </button>
        </div>
        </div>
        </div>
              ...
              <div className="bg-white p-8 rounded-2xl shadow-sm border border-stone-100 flex flex-col items-center justify-center">
                <div className="p-4 bg-white rounded-xl border border-stone-100 mb-4 shadow-inner">
                  {qrValue && <QRCode value={qrValue} size={110} fgColor="#44403c" />}
                </div>
                <p className="text-[9px] font-bold uppercase text-stone-300">QR Code</p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <div className="relative h-44 rounded-xl bg-stone-50 border border-dashed border-stone-200 overflow-hidden group">
                    {profile?.header_bg_url ? (
                      <img src={profile.header_bg_url} className="w-full h-full object-cover" alt="Cover" />
                    ) : (
                      <div className="flex items-center justify-center h-full text-stone-300 text-xs italic">Nessuna immagine di sfondo</div>
                    )}
                    <label className="absolute inset-0 flex items-center justify-center bg-black/30 opacity-0 group-hover:opacity-100 transition-all cursor-pointer text-white text-[10px] font-bold uppercase">
                      Cambia Copertina
                      <input type="file" hidden onChange={(e) => handleFileUpload(e, 'header_covers')} />
                    </label>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="md:col-span-2 space-y-3">
                    ...
                    <select value={fontFamily} onChange={(e) => setFontFamily(e.target.value)} className="w-full h-[54px] bg-stone-50 border border-stone-100 rounded-xl px-4 text-xs font-bold outline-none appearance-none cursor-pointer">
            ...
            {/* GALLERIA IMMAGINI CON NUOVA LOGICA DI DRAG & DROP */}
            <div className="bg-white p-10 rounded-2xl shadow-sm border border-stone-100">
              <div className="flex justify-between items-center mb-10">
                <div className="space-y-1">
                    ...
                    <p className="text-[9px] font-bold text-stone-400 uppercase tracking-widest">Trascina le foto per riordinarle. L'ordine viene salvato automaticamente.</p>
                </div>
                <label className="bg-stone-800 text-white px-5 py-2.5 rounded-xl text-[10px] font-bold uppercase hover:bg-stone-600 cursor-pointer shadow-md transition-all">
                    + Aggiungi Foto <input type="file" hidden multiple onChange={(e) => handleFileUpload(e, 'gallery')} />
                </label>
              </div>
                ...
                {galleryItems
                  ...
                  <div 
                    ...
                    onDragStart={(e) => {
                        ...
                        e.currentTarget.classList.add('opacity-40', 'scale-90');
                    }}
                    onDragEnd={(e) => {
                        e.currentTarget.classList.remove('opacity-40', 'scale-90');
                    }}
                    onDragOver={(e) => {
                        e.preventDefault();
                    ...
                    onDrop={async (e) => {
                        ...
                        // LOGICA RIORDINO LISTA COMPLETA
                        const items = [...galleryItems];
                        const draggedIdx = items.findIndex(i => i.id === draggedId);
                        const targetIdx = items.findIndex(i => i.id === targetId);
                        
                        // Spostiamo l'elemento nell'array
                        const [reorderedItem] = items.splice(draggedIdx, 1);
                        items.splice(targetIdx, 0, reorderedItem);

                        // Assegniamo i nuovi indici di posizione a TUTTI
                        const updatedItems = items.map((img, index) => ({
                            ...img,
                            position: index
                        }));

                        // Aggiorniamo la UI subito
                        setGalleryItems(updatedItems);

                        // Salviamo tutte le posizioni nel DB (Upsert massivo)
                        const { error } = await supabase.from('gallery_items').upsert(
                          updatedItems.map(img => ({
                            id: img.id,
                    ...
                    className="group relative aspect-[4/5] rounded-xl overflow-hidden border border-stone-100 shadow-sm cursor-move bg-stone-50 transition-all active:scale-95"
                  >
                    ...
                    <div className="absolute inset-0 bg-stone-900/10 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none">
                       <span className="bg-white/90 px-3 py-1 rounded-full text-[8px] font-bold uppercase text-stone-700 shadow-sm">Sposta</span>
                    </div>
                    <button 
                      onClick={(e) => { e.preventDefault(); e.stopPropagation(); deleteItem(item); }} 
                      className="absolute top-2 right-2 bg-white/90 text-stone-400 w-7 h-7 rounded flex items-center justify-center opacity-0 group-hover:opacity-100 hover:text-red-600 font-bold transition-all z-30"
                    >
                      ✕
                    </button>
                  </div>
            ...
            <div className="bg-white p-10 rounded-2xl shadow-sm border border-stone-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {galleryItems.filter(i => i.type !== 'image').map((item, i) => (
...
<button onClick={async () => {
    ...
    await supabase.from('gallery_items').insert({ 
      ...
      type: 'video', // Usiamo un tipo generico per tutti i video esterni
      position: galleryItems.length 
    });
    setNewVideoUrl(''); 
    fetchData();
}} 
className="flex-1 bg-stone-800 text-white py-3 rounded-lg text-[10px] font-bold uppercase">
  Salva Link
</button>
                      <label className="flex-1 bg-stone-200 text-stone-600 py-3 rounded-lg text-[10px] font-bold uppercase tracking-widest cursor-pointer text-center">
                          Carica File <input type="file" hidden accept="video/*" onChange={(e) => handleFileUpload(e, 'videos')} />
                      </label>
                    </div>
                </div>
              </div>
